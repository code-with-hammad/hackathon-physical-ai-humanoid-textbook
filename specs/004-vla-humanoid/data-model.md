...
`Voice Command` --> `OpenAI Whisper` --> `Transcribed Text` --> (`Action Mapper` or `LLM`) - If `Action Mapper`: `Transcribed Text` --> `Robot Action` --> `ROS 2 Executable Action` --> `Humanoid Robot` - If `LLM`: `Transcribed Text` + `Perception Feedback` --> `LLM` --> `Cognitive Plan` (sequence of `Robot Action`s) --> `ROS 2 Executable Action` --> `Humanoid Robot`
